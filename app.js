...
async function boot(){
  ...
  if (configured){
    // Live site. Never fall back to sample data here — showing invented writeups on the
    // real site would be worse than showing an honest error.
    if (!(window.supabase && window.supabase.createClient)){
      app.innerHTML = `<section class="page-body" style="padding-top:64px"><p class="eyebrow">Connection problem</p>
        <h2 style="margin-top:18px">The site can't reach its library right now.</h2>
        <p class="lede" style="margin-top:16px">Please refresh the page in a moment. If this continues, contact ${esc(CFG.contactEmail || "the research desk")}.</p></section>`;
      return;
    }
    store = new LiveStore(window.supabase.createClient(CFG.supabaseUrl, CFG.supabaseAnonKey));
    try { await store.init(); } catch (e) { console.error("Supabase init failed:", e); }
